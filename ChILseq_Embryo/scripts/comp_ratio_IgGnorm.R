...
matrix_files <- grep(".*_TSS_matrix.rds", args, value = TRUE)
matrix_files <- grep("S5P|IgG", matrix_files, value = TRUE)
...
my_mats <- ls(pattern = paste0("^TSS.*S5P"))
...
for(i in seq_along(my_mats)){
    ...
    my_mat_IgG <- averageMats(paste0(gsub("S5P|S2P","IgG",gsub("_DRB|[1-3]$", "", my_mats[i])), 1:3))
...
ggp <- ggbarplot(data = df, 
    ...
    ggtitle(label = "S5P/IgG at Q5 genes") +
