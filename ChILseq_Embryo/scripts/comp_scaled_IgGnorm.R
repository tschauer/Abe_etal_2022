...
matrix_files <- grep(".*_T[T,S]S_matrix.rds", args, value = TRUE)
matrix_files <- matrix_files[order(matrix_files)]


stopifnot(length(matrix_files) == 26)
...
for(i in seq_along(my_mats)){
    ...
    my_mat_IgG <- get(gsub("S5P|S2P","IgG",gsub("_DRB", "", my_mats[i])))
    ...
    for(my_scaling in c("scaled","unscaled")){
        ...
        for(my_sample_mats in my_plot_mats){
            ...
            plotComposite(my_sample_mats = my_sample_mats, 
                          ...
                          ylims = c(ifelse(my_scaling == "scaled", 0, 1), 
                                    ifelse(my_scaling == "scaled", 1, 2.05)),
                          my_binning = 1,
            ...
            if(my_scaling == "scaled"){
                mtext(text = paste0("Scaled ", my_assay, " / IgG"), side = 2, line = 3, cex = 1.25)
            } else {
