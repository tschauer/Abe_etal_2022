...
for(my_histone in c("ATACseq", "H3K4me3", "H3K36me3","Pol2")){
    ...
    for(log_type in log_types){
        ...
        for(my_assay in my_assays){
            ...
            lnc_scaled_tmp <- lnc_scaled_tmp[,grep("Zy|2C_S[2,5]P_[1-3]|8C", colnames(lnc_scaled_tmp))]
            ...
            # IgGnorm - no need for scaling
            lnc_scaled_tmp <- lnc_repmeans_tmp 
            ...
            lnc_scaled_tmp <- lnc_scaled_tmp[,grep("Zy|2C_S|8C", colnames(lnc_scaled_tmp))]
            ...
            for(hisStage in  grep("zygote|2C|8C", colnames(hislnc_Qs_tmp), value = TRUE)){
                ...
                ggp <- ggplot(lnc_merged_Qs_tmp_long, 
                    ...
                    coord_cartesian(ylim = c(-4, 4)) +
                    scale_y_continuous(breaks = seq(-4,4,2)) +
                    ylab(label = paste(my_assay,"/",gsub("norm","",log_type)))
            ...
            for(myStage in  colnames(lnc_Qs_tmp)){
                ...
                ggp <- ggplot(lnc_merged_Qs_tmp_long, 
                    ...
                    stat_summary(fun =  "median", geom="point", size=1) +
                    scale_fill_manual(values = vio_colors) +
                    xlab(label = paste(gsub("_",": ",myStage),"/",gsub("norm","",log_type))) +
...
for(log_type in log_types){
    ...
    for(my_assay in rev(my_assays)){
        ...
        log2_TPM_merged_tmp <- merge(log2_TPM, lnc_scaled_tmp, by = "row.names")
