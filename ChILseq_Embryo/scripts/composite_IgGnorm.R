...
my_assay <- gsub(".*_I","I",gsub(".*_S","S",gsub("_DRB","", my_group)))
my_stage <- gsub(paste0("_",gsub("_[1-3]$","",my_assay),"|_[1-3]$"),"", my_group)
...
matrix_files <- c(matrix_files, gsub("/matrix/","/matrix_ave/",gsub("S5P|S2P","IgG",gsub("_DRB|_[1-3]", "", matrix_files))))
...
stopifnot(length(my_mats) == 2)

for(i in seq_along(my_mats)){
        
        my_mat <- get(my_mats[i])
        
        stopifnot(identical(rownames(get(my_mats[1])), rownames(my_mat)))
        stopifnot(sum(grepl("ENS", rownames(my_mat))) == nrow(my_mat))
        
        my_mat_IgG <- get(gsub("S5P|S2P","IgG",gsub("_DRB|_[1-3]", "", my_mats[i])))
        
        stopifnot(identical(rownames(my_mat_IgG), rownames(my_mat)))
        
        my_mat_norm <- (my_mat+0.5) / (my_mat_IgG+0.5)
        
        assign(my_mats[i], my_mat_norm)
        
        rm(list = c("my_mat","my_mat_norm","my_mat_IgG"))
}
...
`res_Cdk9in-Noinject` <- read.table("../RNAseq_Cdk9_Spt5/Output/deseq2/tables/res.Cdk9in-Noinject.txt")
...
`res_Spt5KD-IgG` <- read.table("../RNAseq_Cdk9_Spt5//Output/deseq2/tables/res.Spt5KD-IgG.txt")
...
if(grepl("reps",output_file)){
        ...
        my_grouping_names <- c("log2_TPM_stage","res_Spt5KD-IgG","res_Cdk9in-Noinject")
...
for(i in seq_along(my_grouping_names)){
        ...
        plotComposite(my_sample_mats = my_sample_mats, 
                      ...
                      ylims = c(1,ifelse(my_assay == "S2P",  2.05, 2.05)),
                      my_binning = 1,
        ...
        mtext(text = paste0(my_assay, " / IgG"), side = 2, line = 3, cex = 1.25)
...
output_file2 <- gsub("composite\\.","composite2.",output_file)
...
for(i in seq_along(my_grouping_names)){
        ...
        my_file_name <- ifelse(my_grouping_names[i] == "log2_TPM_stage", 
                               output_file2,
                               paste0(gsub(".pdf", paste0(".",gsub("res_","",my_grouping_names[i]),".pdf"), output_file2)))
        ...
        pdf(file = my_file_name, width = 7.5, height = 4.5)
        ...
        for(my_site in c("TSS", "TTS")){
                ...
                plotComposite(my_sample_mats = my_sample_mats,
                              ...
                              line_lwd = 2,
                              smoother = 251,
                              yaxt = ifelse(my_site %in% c("TSS","TTS"), "s", "n"),
                              log_scale = FALSE,
                              add_line = FALSE,
                              add_range = NULL)
